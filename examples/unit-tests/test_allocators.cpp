...
{
	msplat::container::LinearAllocator allocator(1024);
	...
	if (reinterpret_cast<uintptr_t>(ptr1) % alignof(std::max_align_t) != 0)
		return false;
	if (reinterpret_cast<uintptr_t>(ptr2) % alignof(std::max_align_t) != 0)
		return false;

	// Test usage tracking
	if (allocator.bytes_used() < 300)
		return false;        // At least 300 bytes used
	if (allocator.bytes_remaining() > 724)
		return false;        // At most 724 bytes remaining
	if (allocator.total_size() != 1024)
		return false;
...
TEST(linear_allocator_alignment)
{
	...
	// Check alignments
	if (reinterpret_cast<uintptr_t>(ptr2) % 16 != 0)
		return false;
	if (reinterpret_cast<uintptr_t>(ptr3) % 64 != 0)
		return false;
...
{
	msplat::container::LinearAllocator allocator(100);
...
TEST(linear_allocator_reset)
{
	...
	if (allocator.bytes_used() != 0)
		return false;
	if (allocator.bytes_remaining() != 1024)
		return false;
...
{
	char                               buffer[1024];
	msplat::container::LinearAllocator allocator(buffer, sizeof(buffer));
	...
	if (ptr < buffer || ptr >= buffer + sizeof(buffer))
		return false;
...
{
	msplat::container::StackAllocator allocator(1024);
	...
	if (allocator.bytes_used() >= used_before)
		return false;
...
{
	msplat::container::PoolAllocator<int> allocator(100);        // 100 int-sized chunks

	if (allocator.chunk_count() != 100)
		return false;
	if (allocator.allocated_chunks() != 0)
		return false;
	if (allocator.free_chunks() != 100)
		return false;
	...
	if (allocator.allocated_chunks() != 2)
		return false;
	if (allocator.free_chunks() != 98)
		return false;
...
{
	msplat::container::PoolAllocator<int> allocator(10);
	...
	if (allocator.free_chunks() != 2)
		return false;
	if (allocator.allocated_chunks() != 8)
		return false;
...
TEST(pool_allocator_external_buffer)
{
	...
	msplat::container::PoolAllocator<int> allocator(buffer, buffer_size);

	if (allocator.chunk_count() == 0)
		return false;
...
TEST(memory_write_read_verification)
{
	...
	for (size_t i = 0; i < size; ++i)
	{
		if (ptr[i] != static_cast<char>(i & 0xFF))
			return false;
	}

...
#include <msplat/core/containers/array.h>
...
class FrameArenaResource
{
  private:
	...
	// Backing buffers - using container::array and container::unique_ptr
	container::array<container::unique_ptr<std::byte[]>, NumArenas> mBuffers;

	// Raw storage for monotonic_buffer_resource objects (not constructed yet)
	container::array<container::aligned_storage_t<sizeof(Mon), alignof(Mon)>, NumArenas> mArenaStorage;
  ...
  public:
	    ...
	    mArenaSize(arenaBytes)
	{
		...
		{
			mBuffers[i] = container::make_unique<std::byte[]>(mArenaSize);

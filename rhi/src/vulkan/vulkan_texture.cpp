...
VulkanTexture::VulkanTexture(VkDevice device, VkImage image, VkFormat format, uint32_t width, uint32_t height,
    ...
    device(device), image(image), imageView(VK_NULL_HANDLE), memory(VK_NULL_HANDLE), width(width), height(height), format(VulkanFormatToTexture(format)), ownedBySwapchain(ownedBySwapchain)
{
	// Create image view

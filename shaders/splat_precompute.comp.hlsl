...
void KillSplat(uint splatID)
{
    HWRasterSplat dead;
    dead.centerClip = float4(0.0, 0.0, 2.0, 1.0);
    dead.ndcBasis1  = float2(0.0, 0.0);
    dead.ndcBasis2  = float2(0.0, 0.0);
    dead.color      = float4(0.0, 0.0, 0.0, 0.0);
    preprocessedSplats[splatID] = dead;
}
...
void main(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    float2 ndcBasis1 = mul(rotMat, basis1) * ubo.basisViewport * 2.0 * ubo.inverseFocalAdj;
    float2 ndcBasis2 = mul(rotMat, basis2) * ubo.basisViewport * 2.0 * ubo.inverseFocalAdj;

    // Write preprocessed splat
    HWRasterSplat result;
    result.centerClip = centerClip;
    result.ndcBasis1  = ndcBasis1;
    result.ndcBasis2  = ndcBasis2;
    result.color      = color;
    preprocessedSplats[splatID] = result;
}
